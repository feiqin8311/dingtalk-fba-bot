...
class SkillScaffoldTests(unittest.TestCase):
    ...
    def test_skill_runner_execs_dingtalk_bot_env_python(self) -> None:
        ...
        self.assertIn('env_file="${repo_root}/.env"', runner_text)
        self.assertIn('original_python_override="${DINGTALK_FBA_BOT_PYTHON:-}"', runner_text)
        self.assertIn('original_conda_env_override="${DINGTALK_FBA_BOT_CONDA_ENV:-}"', runner_text)
        self.assertIn('set -a', runner_text)
        self.assertIn('source "${env_file}"', runner_text)
        self.assertIn('set +a', runner_text)
        self.assertIn('export DINGTALK_FBA_BOT_PYTHON="${original_python_override}"', runner_text)
        self.assertIn('export DINGTALK_FBA_BOT_CONDA_ENV="${original_conda_env_override}"', runner_text)
