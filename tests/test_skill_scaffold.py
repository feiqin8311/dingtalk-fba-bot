...
class SkillScaffoldTests(unittest.TestCase):
    ...
    def test_ops_wrapper_execs_dingtalk_bot_env_python(self) -> None:
        ops_runner_text = (SKILL_ROOT / "scripts" / "run-fba-alert.sh").read_text(encoding="utf-8")

        self.assertIn("python_bin=", ops_runner_text)
        self.assertIn("/envs/dingtalk-bot/bin/python", ops_runner_text)
        self.assertIn('exec env \\', ops_runner_text)
    ...
    def test_skill_documents_japan_scoped_trigger_for_ops_wrapper(self) -> None:
        ops_skill_text = (SKILL_ROOT / "SKILL.md").read_text(encoding="utf-8")
        ops_reference_text = (SKILL_ROOT / "references" / "config.md").read_text(encoding="utf-8")

        self.assertIn("-日本", ops_skill_text)
        self.assertIn("--scope us|ca|eu|jp", ops_skill_text)
