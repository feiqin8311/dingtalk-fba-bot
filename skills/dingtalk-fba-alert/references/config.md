...
bash skills/dingtalk-fba-alert/scripts/run-fba-alert.sh --dry-run --scope ezarc
bash skills/dingtalk-fba-alert/scripts/run-fba-alert.sh --dry-run --scope yplus
bash skills/dingtalk-fba-alert/scripts/run-fba-alert.sh --dry-run --scope ezarc-test
bash skills/dingtalk-fba-alert/scripts/run-fba-alert.sh --dry-run --scope yplus-test
```
...
bash skills/dingtalk-fba-alert/scripts/run-fba-alert.sh --scope ezarc
bash skills/dingtalk-fba-alert/scripts/run-fba-alert.sh --scope yplus
bash skills/dingtalk-fba-alert/scripts/run-fba-alert.sh --scope ezarc-test
bash skills/dingtalk-fba-alert/scripts/run-fba-alert.sh --scope yplus-test
```
...
bash skills/dingtalk-fba-alert/scripts/run-fba-alert.sh --scope ezarc --notify-user-id <sender_id>
bash skills/dingtalk-fba-alert/scripts/run-fba-alert.sh --scope yplus --notify-user-id <sender_id>

Upload without DingTalk messages:

```bash
bash skills/dingtalk-fba-alert/scripts/run-fba-alert.sh --scope ezarc --upload-only
```
```
...
- `DINGTALK_ROBOT_CODE`
- `DINGTALK_USER_IDS`
- `DINGTALK_FBA_BOT_PYTHON`: optional absolute path to the Python environment used by the wrapper. The wrapper reads it from `.env` and lets an already-exported value take precedence; otherwise it uses `python3` from the current environment.
...
- `EZARC库存预警` and `YPLUS库存预警` map to `--scope ezarc` and `--scope yplus`; their `测试` variants map to `--scope ezarc-test` and `--scope yplus-test`.
...
- `--scope ezarc|yplus` generates a brand main report and matching store reports. The `-test` variants use the same scope with test-labelled reports.
- `--upload-only` uploads to DingPan and skips all DingTalk messages; it has no effect in `--dry-run` mode.
- Supplying `--notify-user-id` for `all`, `ezarc`, or `yplus` sends only the main report and skips store-report delivery.
